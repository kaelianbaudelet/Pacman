...
class Lab:
    ...
    def __init__(self, l: list):
        ...
        # compte les pagmme pour savoircombien le joeuur doit manger d epaggomme via uen for qui parcous la grille
        self.total_pagomme = self.comptage_gomme(l)
        self.adjacence = {}
        self.all_gomme_eaten = False
        ...
        for i in range(len(self.grille)):
            for j in range(len(self.grille[0])):
                if self.grille[i][j] == 1:
                    ...
                    pyxel.rect(j*8+1, i*8+1, 6, 6, 7) # porte fantome
    ...
    def detection_gomme(self, x, y):
        # si toute les gomme sont mangées, on met "all_gomme_eaten" à True
        if self.total_pagomme == 0:
            self.all_gomme_eaten = True
        # Si une gomme est détectée, on la mange
        if self.grille[y][x] == 2:
            self.total_pagomme -= 1
            self.grille[y][x] = 0
            return 10
        return 0
