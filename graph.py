...
class Graph:
    ...
    def __init__(self, grid):
        ...
        for i in range(len(grid)):
            for j in range(len(grid[0])):
                ...
                if grid[i][j] != 1:
                    self.graph[(i, j)] = [] # On crée un sommet pour chaque case non mur

                    # On ajoute les arêtes pour chaque case adjacente non mur
                    if i - 1 > -1 and grid[i - 1][j] != 1: # Si la case adjacente en haut n'est pas un mur
                        self.graph[(i, j)].append((i - 1, j)) # On ajoute l'arête entre la case courante et la case adjacente en haut
                    if i + 1 < len(grid) and grid[i + 1][j] != 1: # Si la case adjacente en bas n'est pas un mur
                        self.graph[(i, j)].append((i + 1, j)) # On ajoute l'arête entre la case courante et la case adjacente en bas
                    if j - 1 > -1 and grid[i][j - 1] != 1: # Si la case adjacente à gauche n'est pas un mur
                        self.graph[(i, j)].append((i, j - 1)) # On ajoute l'arête entre la case courante et la case adjacente à gauche
                    if j + 1 < len(grid[0]) and grid[i][j + 1] != 1: # Si la case adjacente à droite n'est pas un mur
                        self.graph[(i, j)].append((i, j + 1)) # On ajoute l'arête entre la case courante et la case adjacente à droite
    ...
    def parcours_largeur(self, som1, som2):
        ...
        marques = {v: False for v in self.graph.keys()} # Dictionnaire des marques
        ...
        # On explore tout le graphe jusqu'a tomber sur le bon sommet, pas besoin de faire tout le parcours
    ...
    def parcours_profondeur(self, som1, som2):
        ...
        # Tant qu'on a pas atteint le sommet de destination et qu'il reste des sommets à explorer
...
def dijkstra(grille, source, cible):
    ...
    distances = [[float('inf')] * m for _ in range(n)] # Initialisation des distances par un nombre infini pour chaque sommet de la grille
    distances[source[0]][source[1]] = 0 # La distance de la source à elle-même est nulle
    ...
    while file:
        ...
        for dx, dy in [(1, 0), (-1, 0), (0, 1), (0, -1)]: # On parcourt les sommets adjacents
            nx, ny = x + dx, y + dy # Coordonnées du sommet adjacent
            if 0 <= nx < n and 0 <= ny < m and grille[nx][ny] != 1: # Si le sommet est dans la grille et n'est pas un mur
                new_dist = dist + 1 # On calcule la nouvelle distance
                if new_dist < distances[nx][ny]: # Si la nouvelle distance est plus petite que l'ancienne
                    distances[nx][ny] = new_dist # On met à jour la distance
                    file.append(((nx, ny), new_dist)) # On ajoute le sommet à la file avec sa nouvelle distance
...
def get_chemin(grille, source, cible):
    ...
    distances = dijkstra(grille, source, cible) # On calcule les distances minimales a l'aide de l'algorithme de Dijkstra
    ...
    while (x, y) != source: 
        ...
        dists = [distances[x + 1][y], distances[x - 1] 
                 [y], distances[x][y + 1], distances[x][y - 1]] # On recupere les distances des sommets adjacents
    ...
    chemin.reverse() # On inverse le chemin pour avoir le bon ordre des sommets du chemin a parcourir
